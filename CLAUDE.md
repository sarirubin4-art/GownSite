...
1. If the change includes an EF migration, apply it to **production** before pushing (GitHub Actions deploys the new code immediately on push, and it will 500 on missing columns otherwise):
   ...
   - Delete `appsettings.local.json` immediately after — confirm it's actually gone (`ls GownSite.Web/appsettings.local.json` should fail) before moving on. **This file existing at all silently redirects every `dotnet ef` command in the repo to whatever DB it points at, with no per-command warning otherwise** — a session on 2026-09-11 forgot to delete it, and a *later* session's routine `dotnet ef database update` meant for local dev landed on production instead (harmless in that case — an additive nullable-safe column — but by luck, not by design). `GownDataContextFactory.cs` now **hard-blocks** (throws, doesn't just warn) any `dotnet ef` command that finds this override file pointing at a connection string that doesn't look like local dev — it refuses to run unless you set `REGOWNED_ALLOW_REMOTE_EF=1` inline on that same command (e.g. `$env:REGOWNED_ALLOW_REMOTE_EF="1"; dotnet ef database update ...`), which is exactly what step 1 here requires you to do deliberately. Don't set that env var as a standing/session-wide variable — pass it inline per command so it can't linger the way the override file itself did.
...
- Stripe is in **live-key mode** (confirmed 2026-09-03) — real payments are being processed.
